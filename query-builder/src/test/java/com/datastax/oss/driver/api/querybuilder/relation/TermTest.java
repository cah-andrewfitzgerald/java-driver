...
public class TermTest {
  ...
  public void should_generate_arithmetic_terms() {
    assertThat(sum(raw("a"), raw("b"))).hasPrettyCql("a + b");
    assertThat(sum(sum(raw("a"), raw("b")), sum(raw("c"), raw("d")))).hasPrettyCql("a + b + c + d");
    assertThat(difference(sum(raw("a"), raw("b")), sum(raw("c"), raw("d"))))
        .hasPrettyCql("a + b - (c + d)");
    assertThat(difference(sum(raw("a"), raw("b")), difference(raw("c"), raw("d"))))
        .hasPrettyCql("a + b - (c - d)");
    assertThat(opposite(sum(raw("a"), raw("b")))).hasPrettyCql("-(a + b)");
    assertThat(opposite(difference(raw("a"), raw("b")))).hasPrettyCql("-(a - b)");
    assertThat(product(sum(raw("a"), raw("b")), sum(raw("c"), raw("d"))))
        .hasPrettyCql("(a + b) * (c + d)");
  ...
  public void should_generate_function_terms() {
    assertThat(function("f")).hasPrettyCql("f()");
    assertThat(function("f", raw("a"), raw("b"))).hasPrettyCql("f(a,b)");
    assertThat(function("ks", "f", raw("a"), raw("b"))).hasPrettyCql("ks.f(a,b)");
  }

  @Test
  public void should_generate_cast_terms() {
    assertThat(cast(raw("1"), DataTypes.BIGINT)).hasPrettyCql("(bigint)1");
  }

  @Test
  public void should_generate_literal_terms() {
    assertThat(literal(1)).hasPrettyCql("1");
    assertThat(literal("foo")).hasPrettyCql("'foo'");
    assertThat(literal(ImmutableList.of(1, 2, 3))).hasPrettyCql("[1,2,3]");
    ...
    assertThat(literal(tupleValue)).hasPrettyCql("(1,'foo')");
    ...
    assertThat(literal(udtValue)).hasPrettyCql("{first_name:'Jane',last_name:'Doe'}");

    assertThat(literal(Charsets.UTF_8, new CharsetCodec())).hasPrettyCql("'UTF-8'");
  }
