...
/** Same as {@link RelationTest}, but using {@code whereXxx()} instead of {@code where(isXxx())}. */
public class SelectFluentRelationTest {
  ...
  public void should_generate_in_relation() {
    assertThat(selectFrom("foo").all().whereColumn("k").in(bindMarker()))
        .hasCql("SELECT * FROM foo WHERE k IN ?");
    assertThat(selectFrom("foo").all().whereColumn("k").in(bindMarker(), bindMarker()))
        .hasCql("SELECT * FROM foo WHERE k IN (?,?)");
  }
